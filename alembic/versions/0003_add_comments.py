...
_TABLES = (
    sa.Table(
        ...
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default=sa.true(), comment="账号是否启用"),
        sa.Column("is_superuser", sa.Boolean(), nullable=False, server_default=sa.false(), comment="是否为超级管理员"),
    ...
    sa.Table(
        ...
        sa.Column("created_by", sa.BigInteger(), nullable=True, comment="上传用户 ID，用户删除后置空"),
        sa.Column("created_at", sa.DateTime(), nullable=False, comment="创建时间（UTC）"),
        sa.Column("updated_at", sa.DateTime(), nullable=False, comment="更新时间（UTC）"),
        comment="文件元数据与存储位置",
    ),
...
def _comments(*, remove: bool) -> None:
    ...
    for table in _TABLES:
        ...
        for column in table.columns:  # type: ignore
            op.alter_column(
                table.name,
                column.name,
                ...
                existing_autoincrement=(column.name == "id" if op.get_context().dialect.name == "mysql" else None),
            )
